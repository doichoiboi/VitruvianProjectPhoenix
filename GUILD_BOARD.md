...
| R-009 | Visual System Refresh | Sightline -> Spanner | `repairs/R009_VisualSystemRefresh.md`; first pass reduces bubbly shapes, thick borders, heavy shadows, and oversized type while keeping existing gradients until a better palette is chosen |
| R-010 | MainViewModel Event Reducer | Spanner -> Sightline | `repairs/R010_MainViewModelEventReducer.md`; introduce `MainViewModelEvent` in small behavior-preserving slices before moving workout execution behind events |
...
| ID | Title | Owner | Note |
|---|---|---|---|
| - | Hardware test checklist | Spanner + Sightline | `HARDWARE_SMOKE_TESTS.md`; running real-machine checklist for R-010 connection safety, BLE, route exit, Stop At Top, Just Lift, and AMRAP smoke passes. |
